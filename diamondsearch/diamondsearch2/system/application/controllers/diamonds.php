...
//		print_r($data);
		...
		if ($details && $details == 'true') {
			...
			if($('#pricerange').slider('value',0) <= 30) {
				val = ($('#pricerange').slider('value',0)*40+(" . $minprice . "));
				$('#pricerange1').val(val);
			}
				...
				$('#pricerange1').val(" . $minprice . ");
	 									 																...
	 									 																if($('#pricerange').slider('value',1) <= 30 && $('#pricerange').slider('value',1) >1)
									 									 																{
									 									 																	val = ((-1)*$('#pricerange').slider('value',1)*40+(" . $maxprice . "));
									 									 																	$('#pricerange2').val(val);
									 									 																}
										 	 ...
										 	 $('#carat').slider({ steps: ".ceil($caratminmax['max']*1000).", min:0, max:".ceil($caratminmax['max']*100).", range: true, slide : function(e,ui) { value = $('#caratmin').val($('#carat').slider('value', 0)/100);
														...
														$(\"#searchresult\").flexigrid
																	...
																	dataType: 'json',
																	colModel : [
																		{display: 'Lot', name : 'lot', width : 100, sortable : false, align: 'left'},
																		{display: 'Shape', name : 'shape', width : 60, sortable : true, align: 'center'},
																		{display: 'Carat', name : 'carat', width : 55, sortable : true, align: 'center'},
																		{display: 'Color', name : 'color', width : 40, sortable : true, align: 'center'},
																		{display: 'Clarity', name : 'clarity', width : 50, sortable : true, align: 'center'},
																		{display: 'Price', name : 'price', width : 65, sortable : true, align: 'center'},
																		{display: 'Ratio', name : 'Ratio', width : 60, sortable : true, align: 'center',hide: true},
																		{display: 'Flurosence', name : 'Flour', width : 60, sortable : true, align: 'center'},
																		{display: 'Cut', name : 'cut', width : 60, sortable : true, align: 'center'},
																		...
																		{display: 'Culet', name : 'Culet', width : 40, sortable : true, align: 'center'},
																		{display: 'Cert', name : 'Cert', width : 55, sortable : true, align: 'center'},
																	...
																	sortorder: \"asc\",
																	usepager: true,
																	title: '<h1 class=\"pageheader\">vDiamonds Search Result</h1>',
																	useRp: true,
																	rp: 40,
																...
																$(\".flexigrid .bDiv\").height(601);
																$(\".flexigrid .nDiv\").height(601);
																$(\".flexigrid .hGrip\").height(601);
...
diamond anniversary band, diamond bridal sets, diamond solitaire pendant, blue diamond jewelry. Purchase discounted rate diamond anniversary band, diamond bridal sets, diamond solitaire pendant,  blue diamond jewelry">
	...
	<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">';
			...
			$data['bodyonload'] = '
				initialize();
				viewDiamondDetails('.$_GET['diamondID'].',"","","");
			';
