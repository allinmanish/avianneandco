...
Fancy Colored Diamonds">
	...
	function engagement_ring_settings($diamondid = true,$addoption = '',$sidestone1 = '', $sidestone2= '', $extraoption = '')
...
if($('#pricerange').slider('value',0) <= 30)
	...
	function getringresults($isPave = true , $Solitaire = true , $Sidestone = true ,$platinum = true, $gold = true,$whitegold = true,$anniversary=true , $minprice = 0 , $maxprice = 1000000 , $shape = 'all', $page = 0,$isMarkt = '',$isErd = '',$isVatche = '',$isDaussi = '',$isAntique = '', $isThreestone = true, $isHalo = true, $isMatching = true, $lot = 0){
				...
				$results = $this->engagementmodel->getRings($start , $rp , $isPave   , $Solitaire  , $Sidestone ,$platinum  , $gold ,$whitegold,$anniversary, $minprice , $maxprice , $shape, $isMarkt, $isErd, $isVatche, $isDaussi, $isAntique, $isThreestone, $isHalo , $isMatching);
				...
				foreach ($results['result'] as $row) {
					...
					$returnhtml .= '
				    				...
				    				'.$row['style'].' collection<br>				    				
							    		 ...
							    		 <center>
								    		 ...
								    		 <img id="ringbigimage'.$row['stock_number'].'" src="';
...

												{
												 ...
												 $img = file_exists(config_item('base_path').'images/rings/carat'.$row['carat_image']) ? config_item('base_url').'images/rings/carat'.$row['carat_image'] : config_item('base_url').'images/rings/noimage.jpg';
...

												{
												    ...
												    $img = file_exists(config_item('base_path').'images/rings/'.$row['small_image']) ? config_item('base_url').'images/rings/'.$row['small_image'] : config_item('base_url').'images/rings/noimage.jpg';
						    		 ...
						    		 <div class="txtsmall"> $'.$row['price'].'-'.$row['metal'].'

						    		 </div>

						    		 <a href="javascript:void(0)" class="search" onclick="viewRingsDetails('.$row['stock_number'].','.$lot.')">View Details</a>
...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">';
	...
	//	exit(0);
		...
		if($addoption == 'tothreestone'){$data['tabhtml'] = $this->commonmodel->getThreeStoneTab('addbasket',$diamondlotno, $sidestone1,$sidestone2,$stockno);}
