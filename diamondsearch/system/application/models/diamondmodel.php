	...
	function getSearch( $page =1 , $rp = 10 ,$sortname = 'title' ,$sortorder = 'desc' ,$query= '' , $qtype = 'title' , $oid = '', $searchtype = ''){
		$results = array();
		  ...
		  else
		  {
			...
			if($cutmin>=0 and $cutmax < count($cutarray)) {
			...
			if(($this->session->userdata('colormin')>=0)) $colormin =  $this->session->userdata('colormin');
			if(($this->session->userdata('colormax'))>=0) $colormax =  $this->session->userdata('colormax');			
			if($colormin>=0 and $colormax < count($colors)) {
			...
			if($claritymin>=0 and $claritymax < count($claritys)) {
			...
			if($fluromin>=0 and $fluromax < count($fluroarray)) {
			...
			if($polismin>=0 and $polismax < count($polisarray)){
			...
			if($symmetmin>=0 and $symmetmax < count($symmetarray)){
