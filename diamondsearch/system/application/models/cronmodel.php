...
function fixhelix()
{
 ...
 function getDiamondByLOT($sku = ''){
	...
	function addDiamondtoEbay($detail, $duration){
		...
		$price = ($detail['price'] + 150.00) * 1.10;
		$watchDetail .='<div>
...
<BR />
		...
		$requestArray['storeCategoryID'] = '3103634018';  
...
//die('tt');
	...
	function sendRequestEbay($requestArray, $section='watches') {
		...
		$requestXmlBody .=  '<Storefront>  
								<StoreCategoryID>3103634018</StoreCategoryID>  
							</Storefront>';
...
//                                fclose($putdata);
	...
	function getAllDiamonds(){
		/* $qry = "SELECT *
				...
				";*/
		$qry = "SELECT *
				FROM ".config_item('table_perfix')."products WHERE price !='0.0' AND ebayid IS NULL  ORDER BY RAND() LIMIT 0,5000
				";
		$return = 	$this->db->query($qry);
		$result = $return->result_array();	
	...
	function getDiamonds(){
		/* $qry = "SELECT *
				...
				";*/
		$qry = "SELECT *
				FROM ".config_item('table_perfix')."products WHERE price !='0.0' AND ebayid IS NULL  ORDER BY RAND() LIMIT 0,1
				";
		$return = 	$this->db->query($qry);
		$result = $return->result_array();	
		return isset($result) ? $result : false;
