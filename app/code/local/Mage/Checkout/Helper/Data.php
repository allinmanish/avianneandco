...
class Mage_Checkout_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function sendPaymentFailedEmail($checkout, $message, $checkoutType = 'onepage')
    {
        ...
        foreach ($checkout->getAllVisibleItems() as $_item) {
            ...
            $items .= $_item->getProduct()->getName() . ' (#' . $_item->getProduct()->getSku() . ')' . '  x '. $_item->getQty() . '  '
        ...
        /* Send payment failed email by wsm */
        
        $templateId = "Payment Failed Customer";
        $emailTemplate = Mage::getModel('core/email_template')->loadByCode($templateId);
        
        $vars = array('customer' => $checkout->getCustomerFirstname() . ' ' . $checkout->getCustomerLastname());
        $emailTemplate->getProcessedTemplate($vars);
        
        $emailTemplate->setSenderName(Mage::getStoreConfig('trans_email/ident_' . Mage::getStoreConfig('checkout/payment_failed/identity', $checkout->getStoreId()) . '/name', $storeId));
        $emailTemplate->setSenderEmail(Mage::getStoreConfig('trans_email/ident_' . Mage::getStoreConfig('checkout/payment_failed/identity', $checkout->getStoreId()) . '/email', $storeId));
        
        $emailTemplate->send($checkout->getCustomerEmail(),$checkout->getCustomerFirstname() . ' ' . $checkout->getCustomerLastname(), $vars);
