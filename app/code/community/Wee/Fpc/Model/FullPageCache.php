...
//		$mob = Mage::helper('awmobile')->getTargetPlatform()=="mobile";
//		$this->_enabled = ($mob)?0:self::isCacheEnabled();
		...
		if (null === $this->_cache) {
			...
			switch ($cacheMethod) {
				case 'file' :
					...
					$options = array (
							'cache_dir' => $cacheDir, /*'file_name_prefix' => self::CACHE_PREFIX*/);
				...
				case 'apc' :
					...
					break;
				case 'memcached' :
					$this->_cache = new Zend_Cache_Backend_Memcached ( array (
							'servers' => array (
									array (
	...
	public function save($output = null) {
		...
		if ($this->_enabled && $this->hasValidLicense () && null !== $output && $isPageCachable) {
			...
			$this->getCache ()->save ( $cacheEntry, $cacheKey, array (
					'wee_fpc' 
			), $cacheLifetime, self::CACHE_PRIORITY );
		}
	}
	...
	protected function _is404() {
		...
		foreach ( $responseHeaders as $responseHeader ) {
			if ($responseHeader ['name'] == 'Http/1.1' && $responseHeader ['value'] == '404 Not Found') {
				$is404 = true;
				break;
			}
		}
	...
	public function getCacheKey() {
		...
		$cacheKeyElements = array (
				...
				'currency_code' => Mage::app ()->getStore ()->getCurrentCurrency ()->getCurrencyCode (), /* 'design' => $design->getPackageName().'_'.$design->getTheme('layout'), */);
